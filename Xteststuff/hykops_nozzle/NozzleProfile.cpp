...
double NozzleProfile::getRelRadius(HLoftElement* element) {
...
void NozzleProfile::setRelRadius(HLoftElement* element, double relRadius) {
		HLoft* loft = element->getHLoft();
		double rNozzle = NozzleInterface::getNozzleRadius(loft);
...
double NozzleProfile::getDiffusorAngle(HLoftElement* element) {
		HCoordinateSystem3D* cos = element->getLoftPlaneDefiningCoordinateSystem();
		if (HCoordinateSystem3D* cos = dynamic_cast<HCoordinateSystem3DAffine*>(cos)) {
			HPoint3D* xi = dynamic_cast<HCoordinateSystem3DAffine*>(cos)->getX1();
...
void NozzleProfile::setDiffusorAngle(HLoftElement* element, double alpha) {
		double chord = getChordlength(element);

		HCoordinateSystem3D* cos = element->getLoftPlaneDefiningCoordinateSystem();
		HPoint3D* horig = dynamic_cast<HCoordinateSystem3DAffine*>(cos)->getOrigin();
...
double NozzleProfile::getChordlength(HLoftElement* element) {
		HCoordinateSystem3D* cos = element->getLoftPlaneDefiningCoordinateSystem();
		if (HCoordinateSystem3D* cos = dynamic_cast<HCoordinateSystem3D*> (cos)) {
			HPoint3D* xi = cos->getOrigin();
...
void NozzleProfile::setChordlength(HLoftElement* element, double chord) {
		...
		HPoint3D* eta = dynamic_cast<HCoordinateSystem3DAffine*>(cos)->getX2();
		double eta1 = eta->getX1() * f;
		double eta2 = eta->getX2() * f;
		double eta3 = eta->getX3() * f;

		HPoint3D* z = dynamic_cast<HCoordinateSystem3DAffine*>(cos)->getX3();
		double z1 = z->getX1();
		double z2 = z->getX2();
		double z3 = z->getX3();

		HCoordinateSystem3D* planeCos = AssembleFactory::createCoordinatesSystem3DAffine(0, horig->getX1(), horig->getX2(), horig->getX3(), xi1, xi2, xi3,  eta1, eta2, eta3, z1, z2, z3);
		planeCos->setHParent(element);
		element->setLoftPlaneDefiningCoordinateSystem(planeCos);
}
